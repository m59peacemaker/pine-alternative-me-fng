...
const publish = async ({ credentials, script, description = "", interactive = false }) => {
	...
	if (!(await elementGetAttribute(page, toggleEditorButtonSelector, 'data-active'))) {
		console.log('not active')
		await page.click(toggleEditorButtonSelector)
		console.log('editor toggle clicked')
		await Promise.all([
			page.waitForSelector(editorSelector, { visible: true }).then(() => console.log('editorsel')),
			waitForElementAttributeValue(page, toggleEditorButtonSelector, 'data-active', 'true')
			.then(() => console.log('data-active'))
		])
	}
	console.log('active')
