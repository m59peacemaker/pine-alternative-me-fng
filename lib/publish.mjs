...
const publish = async ({ credentials, script, description = "", interactive = false }) => {
	const browser = await puppeteer.launch({
		args: [ '--no-sandbox' ], // to make it work on Travis CI
	...
	await page.waitForSelector('.study', { visible: true, timeout: (1.5 * 60 * 1000) })
	...
	if (!interactive) {
		...
		await waitForSelectorNotFound(page, '.test-publish-button')
		await browser.close()
	}
}
